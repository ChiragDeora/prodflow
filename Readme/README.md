...
3. **Access the application:**
   ...
   - Production: http://localhost:3000 (or http://localhost:80 via nginx)

4. **Manage Docker containers:**
   ```bash
   cd helpers/docker
   ./docker-setup.sh stop      # Stop all containers
   ./docker-setup.sh logs dev  # View development logs
   ./docker-setup.sh status    # Check container status
   ```

See [helpers/docker/DOCKER_README.md](helpers/docker/DOCKER_README.md) for detailed Docker documentation.
...
```sql
-- Copy and paste the contents of helpers/sql/setup/setup_fresh_database.sql
-- into your Supabase SQL Editor and execute
```

### Manual Setup
1. **Clear existing data**: Use `helpers/sql/cleanup/purge_tables.sql`
2. **Add machine fields**: Use `helpers/sql/setup/apply_machine_fields.sql`
3. **Complete reset**: Use `helpers/sql/setup/setup_fresh_database.sql`

See [helpers/sql/README.md](helpers/sql/README.md) for detailed documentation.
